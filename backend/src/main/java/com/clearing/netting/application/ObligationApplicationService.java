...
public class ObligationApplicationService {
    ...
    public TradeObligation cancel(String obligationId, String reason) {
        if (reason == null || reason.isBlank()) {
            throw new DomainException("VALIDATION_ERROR", "cancel reason is required");
        }
        ...
        if (obligation.getStatus() != ObligationStatus.OPEN) {
            throw new DomainException("OBLIGATION_NOT_OPEN",
                    "only OPEN obligations can be cancelled, current status: " + obligation.getStatus());
        }
